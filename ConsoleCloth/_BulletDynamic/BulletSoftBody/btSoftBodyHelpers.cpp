													...
													bool bfacesfromtetras)
{
    ...
    FILE *fp = fopen("nodes.data","rb");
    if(fp){
        fread(&nnode,sizeof(int),1,fp);
        pos.resize(nnode);
        fread(&pos[0],sizeof(btVector3),pos.size(),fp);
        //printf("read nodes from binary data\n");
    }
    else{
        //printf("read nodes from inl file\n");
        ...
        fp = NULL;
        fp = fopen("nodes.data","wb");
        if(fp){
            fwrite(&nnode,sizeof(int),1,fp);
            fwrite(&pos[0],sizeof(btVector3),pos.size(),fp);
            //printf("nodes.data write successed!\n");
        }
        else
            printf("nodes.data open failed!\n");
...
if(face&&face[0])
    ...
    if(ele&&ele[0])
    {
        ...
        fp = NULL;
        fp = fopen("elements.data","rb");
        if(fp){
            fread(&ntetra,sizeof(int),1,fp);
            fread(&ncorner,sizeof(int),1,fp);
            fread(&neattrb,sizeof(int),1,fp);

            int *nii = (int *)calloc(sizeof(int),ntetra * 4);
            fread(nii,sizeof(int),ntetra * 4,fp);
            //printf("read elemets from binary data\n");

            for(int i=0;i<ntetra;++i)
            {
                psb->appendTetra(nii[4*i+0],nii[4*i+1],nii[4*i+2],nii[4*i+3]);
                if(btetralinks)
                {
                    psb->appendLink(nii[4*i+0],nii[4*i+1],0,true);
                    psb->appendLink(nii[4*i+1],nii[4*i+2],0,true);
                    psb->appendLink(nii[4*i+2],nii[4*i+0],0,true);
                    psb->appendLink(nii[4*i+0],nii[4*i+3],0,true);
                    psb->appendLink(nii[4*i+1],nii[4*i+3],0,true);
                    psb->appendLink(nii[4*i+2],nii[4*i+3],0,true);
                }
            } // for
            ...
            printf("read elements from in inl\n");
            sscanf(ele,"%d %d %d",&ntetra,&ncorner,&neattrb);
            ele += nextLine(ele);

            int *nei = (int *)calloc(sizeof(int),ntetra * 4);
            for(int i=0;i<ntetra;++i)
            {
                int			index=0;
                int			ni[4];
                ...
                nei[4*i+0] = ni[0];nei[4*i+1] = ni[1];nei[4*i+2] = ni[2];nei[4*i+3] = ni[3];
            ...
            fp = NULL;
            fp = fopen("elements.data","wb");
            if(fp){
                fwrite(&ntetra,sizeof(int),1,fp);
                fwrite(&ncorner,sizeof(int),1,fp);
                fwrite(&neattrb,sizeof(int),1,fp);

                fwrite(nei,sizeof(int),ntetra * 4,fp);
                //printf("elements.data write successed\n");
            }
            else
                printf("elements.data open failed\n");

            free(nei);
        }
    }

    printf("Nodes:%u, Faces:%u, Links:%u, Tetras:%u\n",psb->m_nodes.size(),psb->m_faces.size(),
                                                       psb->m_links.size(),psb->m_tetras.size());
    //printf("Links:  %u\r\n",psb->m_links.size());
    //printf("Faces:  %u\r\n",psb->m_faces.size());
    //printf("Tetras: %u\r\n",psb->m_tetras.size());
    return(psb);
