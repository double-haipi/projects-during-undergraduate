...
void				btSoftBody::PSolve_Links(btSoftBody* psb,btScalar kst,btScalar ti)
{
    ...
    // volume conservation \\ Modified By ZhangJG
    for(int j=0, nj = psb->m_tetras.size(); j<nj; ++j)
    {
        //break;
        Tetra&	t=psb->m_tetras[j];
        btVector3 p1 = t.m_n[0]->m_x;
        btVector3 p2 = t.m_n[1]->m_x;
        btVector3 p3 = t.m_n[2]->m_x;
        btVector3 p4 = t.m_n[3]->m_x;
        // the constraint function
        btScalar Cp1p2p3p4 = 1.0 / 1 * (p2-p1).cross(p3-p1).dot(p4-p1) - t.m_rv;
        btVector3 delta_p1Vol = 1.0 / 1 * (p4-p2).cross(p3-p2);
        btVector3 delta_p2Vol = 1.0 / 1 * (p3-p1).cross(p4-p1);
        btVector3 delta_p3Vol = 1.0 / 1 * (p4-p1).cross(p2-p1);
        btVector3 delta_p4Vol = 1.0 / 1 * (p2-p1).cross(p3-p1);

        btScalar sFenmu = t.m_n[0]->m_im * delta_p1Vol.length2() + t.m_n[1]->m_im * delta_p2Vol.length2()
                        + t.m_n[2]->m_im * delta_p3Vol.length2() + t.m_n[3]->m_im * delta_p4Vol.length2();

        //if(sFenmu < 0.002)
        //    continue;

        btScalar tScale = Cp1p2p3p4 / sFenmu;

        t.m_n[0]->m_x -= t.m_n[0]->m_im * tScale * delta_p1Vol;
        t.m_n[1]->m_x -= t.m_n[1]->m_im * tScale * delta_p2Vol;
        t.m_n[2]->m_x -= t.m_n[2]->m_im * tScale * delta_p3Vol;
        t.m_n[3]->m_x -= t.m_n[3]->m_im * tScale * delta_p4Vol;
    }
}
